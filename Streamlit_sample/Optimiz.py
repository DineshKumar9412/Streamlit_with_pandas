...
#
# with st.container():
#     fls = st.multiselect("you selected : {}".format(x), df[x].unique(),key='1')
#
# gh = df[df[x].isin(fls)]
#
# fig = px.histogram(gh, x="City", y="Sales",color="Segment",barmode="group")
# fig.update_layout({
...
#
# with st.container():
#     fls = st.multiselect("you selected : {}".format(x), df[x].unique(),key='2')
#
# gh = df[df[x].isin(fls)]
#
# fig = px.histogram(gh, x="City", y="Sales",color="Segment",barmode="group")
...
if st.sidebar.checkbox("View 2"):
    ...
    def new():
        ...
        with placeholders.container():
            sd = st.radio("", ['Ascending', 'Descending'], key="88")
            st.write(
                '<style>div.row-widget.stRadio > div{flex-direction:row;border-radius: 15px 15px; height: 10px; width: 200000px;}</style>',
                unsafe_allow_html=True)
