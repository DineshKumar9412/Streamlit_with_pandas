...
def big_data():
    df = pd.read_csv("dashhmis.csv")
    return df
dfs = big_data()

df = dfs[["Directorate Name", "Facility Type", "Total Facility Count","Total Live Facility Count", "Total User Count", "User Login Count", "Users Login  %","Registration Count","Lab Order Count","Admission Count","Total Department Count","Total Active Department Count","Department Using %","Total Doctor Count","Total Doctor Login Count","Doctor Login %","Total Nurse Count","Total Nurse Login Count","Nurse Login %"]]
...
with col1: #fin.iloc[0].tolist()[2]
    fin = 22
    st.markdown(f'<p style="color:black;">Max sale of the Product</p>', unsafe_allow_html=True)
    st.markdown(
        f"<h1 style='text-align: center; color: red; background-color:ivory;font-family:verdana;border-color:black; border: solid; border-radius: 15px 15px;border-width: 5px 5px; height:fixed; width: fixed;font-size:40px;'>{fin}</h1>",
        unsafe_allow_html=True)
with col2:
    ...
    if option == "No":
        ...
        if st.sidebar.checkbox("Display bar chart",key='1'):
            ...
            st.write(
                ...
                unsafe_allow_html=True)
            if sd == "None":
                fig = px.histogram(df, x=x, y=y,color=c,color_discrete_sequence=px.colors.cyclical.Twilight,title="<b>Sales by city </b> <br>Overview of sales by city or cities in a state for the year 2017")
                fig.update_layout({
            ...
            with st.empty():
                ...
                if len(fg) > 0:
                    ...
                    if len(dfm) > 0:
                        ...
                        #     axis=1
                        # )
                        dg = fgs.groupby(["facility_name"])['Registration Count'].sum().reset_index()
                        # print(dg.colu)
                        # dfj = pd.merge(mnth_df, dg, how='outer', on='Month').replace(np.nan, 0)
                        # dfd = dfj.round(decimals=2)
                        fig = px.line(dg, x="facility_name", y="Registration Count", text="Registration Count", markers=True,title = "Line chart by the Month and Sales")
