...
with col1:
    st.markdown(f'<p style="color:black;">User login in percentage</p>', unsafe_allow_html=True)
    st.markdown(
        ...
        unsafe_allow_html=True)
with col2:
    st.markdown(f'<p style="color:black;">Department usage in percentage</p>', unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        unsafe_allow_html=True)
with col3:
    st.markdown(f'<p style="color:black;">Doctor login in percentage</p>', unsafe_allow_html=True)
    # number1 = city
    st.markdown(
        ...
        unsafe_allow_html=True)
with col4:
    st.markdown(f'<p style="color:black;">Nurse login in percentage</p>', unsafe_allow_html=True)
    st.markdown(
...
if st.sidebar.checkbox("View by Metric"):

    col, col2 = st.columns([15, 1])
    with col:
        fls = st.selectbox("Please select the Metric", ["None", "Users Login  %","Facility Live count %","Department Using %", "Doctor Login %", "Nurse Login %"], key='2')
        if fls == "None":
        ...
        # elif fls == "Users Login  %":
            ...
            if sd == "None":
                ...
                if st.radio("View Data", ["No","Yes"]) == "Yes":
                    ...
                    st.markdown("<br>", unsafe_allow_html=True)
                    st.write("Nothing to show, if you need to see data Select Yes")
if st.sidebar.checkbox("View by Directorate"):
    sumry = pd.read_csv("/home/troondxadmin/aimp/csv_file/DMS.csv")
    col, col2 = st.columns([15, 1])
    with col:
    ...
    def new():
        ...
        with st.empty():
            fig = px.bar(ddd, x="Metric", y="Percentage", color="Directorate Name", barmode="relative", text_auto=True,
            ...
            if len(dfms) > 0:
                ...
                dgh = pd.read_csv("/home/troondxadmin/aimp/csv_file/DMS1.csv")
